...
import type { Column } from './compras-types';
...
export default function ComprasDataTable({ columns, items, onRowAction, detailField }: Props) {
                    ...
                    {sorted.length === 0 ? (
                        ...
                        sorted.map((row, i) => (
                            <TableRow
                                ...
                                sx={{ '&:hover': { bgcolor: 'action.hover' }, cursor: detailField ? 'pointer' : undefined }}
                                onClick={() => detailField && onRowAction?.('detail', row)}
                            >
                                {columns.map((col) => (
                                    <TableCell
                                        key={col.key}
